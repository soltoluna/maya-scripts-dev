...
> **その後 (2026-08-16)**: `maya-scripts-dev` はモノレポ化に伴い **後者を採用し、`_REMOTE_FILES` を廃止した**。
> ハブ 1 本が tree walk で全ツールを列挙する形にすると API コールがツール数に比例しなくなり、
> 自動列挙のコストが一気に下がる。詳細は **§2-6**。この事故は原理的に起きなくなったが、
> 配布対象を拡張子で絞っている以上「未登録の拡張子だけ届かない」形で**穴は残る**。
...
**根本的な逃げ道はオフライン インストール。** 入口が 2 つある。

| 経路 | 合図 | 実機確認 |
|---|---|---|
| **配布バンドル**（`tools/make_bundle.py` の ZIP） | 隣の `maya_tools_offline.txt` | **2026-09-16 済み**（Maya 2024） |
| ネットワーク失敗時の確認ダイアログ（`_offline_fallback`） | GitHub への接続失敗 | **未確認** |

配る側が `make_bundle.py` で ZIP を作って渡すのが確実（相手はドラッグ&ドロップ
するだけで、確認ダイアログも出ず、**渡したツールだけが入る**）。 相手が自分で
リポジトリの ZIP を落とした場合は後者の経路になる。 どちらもネットワークに
一切触れないので、ネットワーク側の解決を待たずに作業を止めないで済む。
...
### 2-6. モノレポなら install.py は **リポジトリに 1 本** にする

§2-1 〜 §2-5 は「1 ツール = 1 リポジトリ」を前提にしている。**ツールを 1 つのリポジトリに集めた時点で、install.py をツールごとに持つ理由は無くなる。**
`maya-scripts-dev` は 2026-08-16 にリポジトリ直下の 1 本へ集約した。

**ハブがやること** (§2-2 との差分だけ):

1. `branches/<branch>` で SHA を取る (従来どおり)
2. **`git/trees/<sha>?recursive=1` を 1 回叩き、リポジトリ全体のパス一覧を取る**
3. `^(?P<name>[^/]+)/(?P=name)/(?P<rel>.+)$` に一致し、`__init__.py` を含むフォルダを **ツールと見なす**
4. 全ツールのファイルを**まとめてメモリに揃えてから**書く
5. ツールごとにシェルフボタンを貼る。ラベルは各 `__init__.py` の `SHELF_LABEL` を正規表現で読む
...
| | ツールごと | ハブ 1 本 |
|---|---|---|
| API コール | ツール数 × 1 | **常に 2** (SHA + tree) |
| 配布リスト | `_REMOTE_FILES` を手で同期 (§1-10) | **宣言そのものが無い** |
| 新ツールの登録 | install.py を書く | 不要 (形が合っていれば拾われる) |
| エンドユーザー | ツールごとにドラッグ | **1 回のドラッグで全部** |

**API コールの数が効く理由**: 未認証の GitHub API は **1 時間 60 回 / IP**。スタジオは NAT で全員が同じ枠を共有するので、「ツールが増えるほど更新が 403 になりやすい」という形で効いてくる。ハブなら本数に依存しない。

**注意点**

- **ハブはどのパッケージにも import 依存を持たせない。** §2-4 の「セルフコンテインな snippet」と同じ理由 — パッケージが壊れていて import できないときに、直すための更新まで止まると詰む
- **単一障害点にはならない。** シェルフの Update も UI の更新ボタンも、毎回 GitHub からハブを取り直して `exec` する。ハブに不具合があれば push すれば次の更新で直る
- **tree のレスポンスが `truncated: true` で返る場合がある** (10 万エントリ / 7MB 超)。ツール用リポジトリで踏むことはまずないが、黙って一部だけ配ると原因不明の `ModuleNotFoundError` になるので、検出して例外にする
- **配布対象は拡張子で絞る** (`_ALLOWED_SUFFIXES`)。絞らないと `.blend` や動画が紛れ込んで更新が重くなる。代わりに「パッケージ内の全ファイルが対象拡張子に入っているか」をテストで突き合わせる — これが §1-10 の検査の後継になる
