...
```powershell
cd D:\maya\scripts\dev\color_override\tests
...
配布の仕組みそのもの（ハブの探索規則・配布対象の拡張子）は**リポジトリ直下の
`tests/`** で検査する。 こちらでは `maya` スタブが入っているため、`install.py` を
読み込むと末尾の自動実行が本当に GitHub へ取りに行ってしまう。
...
テストは、スタブを入れないリポジトリ直下の `tests/` に置く。
