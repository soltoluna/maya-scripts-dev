...
- **対応 Maya は 2023（Python 3.9）を基準**にする。 コードは **Python 3.9 で動く構文**に
  留める（`match` 文、`int | None` の実行時評価、3.10+ の標準ライブラリ API は使わない。
  型注釈を書くなら `from __future__ import annotations` を付ける）
  - 会社の Maya が別バージョンなら、まずこの行を直す。 Maya 2022=3.7 / 2023=3.9 /
    2024=3.10 / 2025・2026=3.11。 **2025 以降は Qt が PySide6 になる**ので、
    PySide を使うツールはバージョンをまたげない
  - 雛形は `maya.cmds` だけで書いてあり Qt に依存しないので、バージョン差の影響を受けない
- 開発機（自宅）: **Maya は入っていない。** テストは Maya スタブで回す（`tests/`）
- 実機（会社）: Maya 本体。 **実機確認はここでしかできない**
- ワークスペース: `D:\maya\scripts\dev`
- GitHub: `soltoluna/maya-tools`（**モノレポ**。 ツール 1 本 = フォルダ 1 つ）
