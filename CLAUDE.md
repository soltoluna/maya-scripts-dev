...
**実機の戻り値や API が分からないときは、推測で直さない。** 出力を貼って
もらえる調査関数（例: `color_override.diagnose()` / `probe_render_setup()`）を
先に push し、実データを見てから書く。 調査関数は**既定でシーンを変更しない**・
**途中で例外を上げず `<ERROR ...>` を値にして最後まで出す**形にする。
（2026-09 に `color_override` のフェース割り当ての読み取りを推測で 3 回外し、
調査関数に切り替えたら 2 往復で API が確定した。）

**安全弁で主機能を止めない。** 「戻せなくなるくらいなら掛けない」型の判断は、
実機で誤発火しても自宅では気付けず、ツールが丸ごと動かなくなる
（同件で色が一切乗らなくなった）。 読めないときは警告して従来の動作に落とす。
...
- **社内では「ブラウザは通るのに Maya の Python だけ GitHub に届かない」人が出る**
  （TLS 傍受プロキシ / PAC / セキュリティソフト。 `_ssl.c:997` の SSL エラー）。
  ハブは接続に失敗すると**隣のフォルダから入れるか確認を出す**ので、ZIP を
  展開してその中の `install.py` をドラッグしてもらう。 切り分けと対処は
  `docs/MAYA_HOT_UPDATE_PATTERNS.md` §1-12。 **検証を切る回避は入れない**
...
- **1 本だけ渡したい / 相手がオフラインのときは
  `python tools\make_bundle.py <ツール名>`。** 指定したツールと `install.py` と
  `maya_tools_offline.txt`（この 1 枚があるとハブは GitHub を見ない）を
  まとめた ZIP が `dist/` に出る。 相手はドラッグ&ドロップするだけ。
  **ただしバンドルから入れても「GitHub から更新」は全ツールを入れる**ので、
  1 本だけ渡した意図を保つなら更新も ZIP を渡し直す
